...
def get_urls():
    ...
    for i in range(1,23): ###23
        ...
        url_1 = "https://www.amazon.com/s/ref=?rh=n%3A228013%2Ck%3Amaster%20lock&page="+ str(i) +"&keywords=master%20lock"
        print(type(url_1))
        print("url_1", url_1)
        print(headers)
        ...
        for j in range(3,33): ###33
            ...
            url_3 = 'https://www.amazon.com/ask/questions/asin/' + asins[j] + '/1/ref=ask_ql_psf_ql_hza'
            print(url_3)
            ...
            # r_review= requests.get(url_3, headers=headers, verify=False, proxies={"http": proxy, "https": proxy}, timeout = 5)
            print(url_3, r_review)
            soup_3 = BeautifulSoup(r_review.text)
            question_page = soup_3.find_all('li', attrs={'class':'a-normal'})
            if question_page == []:
                continue
            else:
                page_1 = int(question_page[-1].text)
                for l in range(1, page_1 + 1):
                    url_2 = 'https://www.amazon.com/ask/questions/asin/' + asins[j] + '/' + str(l) +'/ref=ask_ql_psf_ql_hza'
                    urls.append(url_2)
    ...
    for i in range(5):
        ...
        try:
            ...
            if r_2.status_code == 200:
                ...
                soup_question = soup_2.find_all('div', attrs={'class':'a-fixed-left-grid a-spacing-small'})
                len_question = len(soup_question)
                ...
                if len_question == 1000:
                    print('jumping out of loop')
                    return{"error":"failed to process the page",}
                else:
                    # For each page, get all reviews and append them in a list of dictionaries
                    for m in range(len_question):
                        soup_image = soup_2.find_all('img')
                        image = soup_image[2]['src']
                        soup_name = soup_2.find_all('a', attrs={'class':'a-size-large a-link-normal'})
                        # soup_name = soup_2.find_all('h1', attrs={'class':'a-size-large a-text-ellipsis'})
                        name = soup_name[0].text.strip()
                        # title = soup_title[m].text
                        # rating = soup_rating[m].text
                        # date = soup_date[m].text
                        try:
                            question = ' '.join(soup_question[m].text.replace('\n', '').split()).replace('Question: ', '').replace(' Customers are chatting about this View conversation', '')
                        except:
                            questoin = ''
                        product_question={
                        'image':image,
                        ...
                        review_list.append(product_question)
...
if __name__=="__main__":
    ...
    with open('amazon_question_results.json','w') as fp:
